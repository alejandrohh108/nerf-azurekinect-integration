...
# --- INICIO NUEVO CÓDIGO: lectura y procesamiento de archivos .yuv ---

def read_yuv_file(filename, width, height):
    frame_size = int(width) * int(height) * 2  # 2 bytes por píxel (16 bits)
    with open(filename, 'rb') as f:
        yuv_data = np.frombuffer(f.read(frame_size), dtype=np.uint16)
    yuv_data = yuv_data.reshape((int(height), int(width)))
    return yuv_data

def load_yuv_depth(H, W, basedir, bd_factor=.75, factor = 1):
    # Lista para almacenar todas las imágenes
    images = []

    directory = str(Path(basedir) / 'Depth_align_undistort')
    
    # Iterar sobre los archivos en el directorio
    for filename in sorted(os.listdir(directory)):
        if filename.endswith('.yuv'):
            filepath = os.path.join(directory, filename)
            yuv_image = read_yuv_file(filepath, W, H)
            images.append(yuv_image)

    # Convertir la lista de imágenes a un array 3D (profundidad, altura, anchura)
    yuv_images = np.stack(images, axis=0)
    yuv_images = yuv_images / 1000
    yuv_images = yuv_images /0.795
    
    i, j = np.meshgrid(np.arange(W, dtype=np.float32), np.arange(H, dtype=np.float32), indexing='xy')
    i_expanded = i[np.newaxis, :, :]
    j_expanded = j[np.newaxis, :, :]
    i_repeated = np.repeat(i_expanded, yuv_images.shape[0], axis=0)
    j_repeated = np.repeat(j_expanded, yuv_images.shape[0], axis=0)
    
    yuv_images_expanded = yuv_images[:, :, :, np.newaxis]
    
    combined = np.concatenate((i_repeated[:, :, :, np.newaxis], j_repeated[:, :, :, np.newaxis], yuv_images_expanded), axis=3)
    
    yuv_filtered_combined_list = []
    yuv_data_list = []
    # Filtrar puntos para cada imagen individualmente
    for img_idx in range(yuv_images.shape[0]):
        # Crear una máscara donde la profundidad no sea cero para la imagen actual
        mask = combined[img_idx, :, :, 2] != 0
        
        # Aplicar la máscara y seleccionar los puntos correspondientes
        filtered_points = combined[img_idx][mask]
        
        # Añadir los puntos filtrados a la lista
        yuv_filtered_combined_list.append(filtered_points)
        
        depth_list = filtered_points[:, 2]
        coord_list = filtered_points[:, :2]
        
        yuv_data_list.append({"depth": np.array(depth_list), "coord": np.array(coord_list)})

    # Convertir la lista de arrays a un array de objetos dtype=object para manejar la variabilidad en la dimensión x
    yuv_filtered_combined_array = np.array(yuv_filtered_combined_list, dtype=object)
    
    return yuv_filtered_combined_array , yuv_data_list
...
def load_colmap_depth_txt(basedir, factor=8, bd_factor=0.75):
    data_file = Path(basedir) / 'colmap_depth.npy'
    
    images = read_images_text(Path(basedir) / 'sparse' / '0' / 'images.txt')
    points = read_points3D_text(Path(basedir) / 'sparse' / '0' / 'points3D.txt')
    ...
    # print(bds_raw.shape)
    # Rescale if bd_factor is provided
    sc = 1. if bd_factor is None else 1./(bds_raw.min() * bd_factor)   ##############3
    ...
    for id_im in range(1, len(images)+1):
        ...
        for i in range(len(images[id_im].xys)):
            ...
            if id_3D == -1:
                continue
            point3dfull = points[id_3D]
            point3D = points[id_3D].xyz
            rotacion = poses[id_im-1,:3,2].T
            traslacion = poses[id_im-1,:3,3]
            punto = (point3D - traslacion)
            depth = (rotacion @ punto)
            depth = depth * sc
            if depth < bds_raw[id_im-1,0] * sc or depth > bds_raw[id_im-1,1] * sc:
                continue
            err = points[id_3D].error
            ...
            weight_list.append(weight)
        if len(depth_list) > 0:
            #imageio.imwrite("output"+str(id_im)+".png", depth_list)
            print(id_im, len(depth_list), np.min(depth_list), np.max(depth_list), np.mean(depth_list))
            data_list.append({"depth":np.array(depth_list), "coord":np.array(coord_list), "error":np.array(weight_list)})
        else:
            print(id_im, len(depth_list))
    # json.dump(data_list, open(data_file, "w"))
    np.save(data_file, data_list)
    return data_list                                #para cada imagen, 
