...
# https://www.youtube.com/watch?v=sPQzudD-83c
...
from load_llff import load_llff_data, load_colmap_depth, load_colmap_llff, load_colmap_depth_txt, load_yuv_depth
...
def getDepth(image):
    ...
    # Normalización: lo más cercano será 1 (blanco) y lo más lejano 0 (negro)
    normalized_depth = (image- min_depth) / (max_depth - min_depth)

    # Invertir los colores: lo más cercano (menor valor) será blanco y lo más lejano será negro
    # Opción 1: usando colormap inverso
    plt.imshow(normalized_depth, cmap='gray', vmin=0, vmax=1)

    # Eliminar los ejes
    plt.axis('off')

    plt.savefig('imagen_depth.png', dpi=300, bbox_inches='tight', pad_inches=0.1)

    # Cerrar la figura para liberar memoria
    plt.close()

    '''# Abrir la imagen guardada y redimensionarla con Pillow
    image = Image.open('imagen_depth.png')

    # Obtener el tamaño actual
    width, height = image.size

    # Redimensionar a la mitad de la resolución original
    new_size = (width // 2, height // 2)
    resized_image = image.resize(new_size, Image.Resampling.LANCZOS)

    # Guardar la imagen redimensionada
    resized_image.save('imagen_depth_reducida.png')'''
...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    monitor = TrainingMonitor("NeRF_mine_mod_v1_RENDER.txt")                       #    NUEVO
    monitor.start()                                                                #    NUEVO
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        monitor.log_iteration_render(time.time() - t)               # NUEVO
...
def train():
    ...
    if args.dataset_type == 'colmap_llff':
        ...
        if args.colmap_depth:
            depth_gts = load_colmap_depth_txt(args.datadir, factor=args.factor, bd_factor=.75)
        images, poses, bds, render_poses, i_test = load_llff_data(args.datadir, args.factor,
                                                                  recenter=True, bd_factor=.75,
                                                                  spherify=args.spherify)
        
        # --- INICIO NUEVO CÓDIGO: carga de profundidad desde YUV ---
        hwf = poses[0,:3,-1]
        poses = poses[:,:3,:4]
        H, W, focal = hwf
        
        yuv_depth, yuv_data_list = load_yuv_depth(H, W, args.datadir, bd_factor=.75, factor = args.factor)
         # --- FIN NUEVO CÓDIGO ---
        ...
        # --- INICIO NUEVO CÓDIGO: establecer límites near/far con yuv_depth ---
        print('DEFINING BOUNDS')
        if args.no_ndc:
            min_depth = np.inf
            max_depth = -np.inf
            for img in yuv_depth:
                depth_values = img[:,2]
                min_depth_img = np.min(depth_values)
                max_depth_img = np.max(depth_values)
                min_depth = min(min_depth, min_depth_img)
                max_depth = max(max_depth, max_depth_img)
    ...
    # --- INICIO NUEVO CÓDIGO: configuración de rayos YUV, COLMAP y RGB ---
    # Prepare raybatch tensor if batching random rays
    N_yuv_depth = int(args.N_rand / 3)
    if not args.colmap_depth:
        N_rgb = args.N_rand
    else:
        N_depth = int(args.N_rand * args.depth_rays_prop)
        N_rgb = args.N_rand - N_depth - N_yuv_depth
    # --- FIN NUEVO CÓDIGO ---
    ...
    if use_batching:
        ...
        print('get rays')
        rays = np.stack([get_rays_np(H, W, focal, p) for p in poses[:,:3,:4]], 0) # [N, ro+rd, H, W, 3]     # utiliza la distancia focal y no la matriz de intrinsecos para calcular las direcciones de los rayos
        if args.debug:
        ...
        if args.colmap_depth:
            ...
            rays_depth_list = []
            for i in i_train:
                rays_depth = np.stack(get_rays_by_coord_np(H, W, focal, poses[i,:3,:4], depth_gts[i]['coord']), axis=0) # 2 x N x 3  (rays_o, rays_d)
                ...
                rays_depth = np.concatenate([rays_depth, depth_value, weights], axis=1) # N x 4 x 3     //rays_o, rays_d, depth, weights together
    ...
    # --- INICIO NUEVO CÓDIGO: generación de rayos desde datos YUV --- 
    yuv_rays_depth = None
    yuv_rays_depth_list = []
    for i in i_train:
        yuv_rays_depth = np.stack(get_rays_by_coord_np(H, W, focal, poses[i,:3,:4], yuv_data_list[i]['coord']), axis=0) # 2 x N x 3  (rays_o, rays_d)
        yuv_rays_depth = np.transpose(yuv_rays_depth, [1,0,2])
        yuv_depth_value = np.repeat(yuv_data_list[i]['depth'][:,None,None], 3, axis=2) # N x 1 x 3
        yuv_rays_depth = np.concatenate([yuv_rays_depth, yuv_depth_value], axis=1)
        yuv_rays_depth_list.append(yuv_rays_depth)
                
    yuv_rays_depth = np.concatenate(yuv_rays_depth_list, axis=0)
    yuv_rays_depth = yuv_rays_depth.astype(np.float32)
    print('yuv_rays_depth.shape:', yuv_rays_depth.shape)
    print('shuffle yuv depth rays')
    np.random.shuffle(yuv_rays_depth)
    print('done')     
    # --- FIN NUEVO CÓDIGO ---
    ...
    if use_batching:
        ...
        # --- INICIO NUEVO CÓDIGO ---
        raysYuvDepth_iter = iter(DataLoader(RayDataset(yuv_rays_depth), batch_size = N_yuv_depth, shuffle=True, num_workers=0)) if yuv_rays_depth is not None else None
        # --- FIN NUEVO CÓDIGO ---
    ...
    monitor = TrainingMonitor("Nombre del archivo .txt donde se escribe la monitorización")                           # NUEVO
    monitor.start()                                                                                                   # NUEVO
    ...
    for i in trange(start, N_iters):
        ...
        if use_batching:
            ...
            # --- INICIO NUEVO CÓDIGO: obtención de batch_yuv_depth ---
            try:
                if len(raysYuvDepth_iter) >= N_yuv_depth:
                       batch_yuv_depth = next(raysYuvDepth_iter).to(device)
                else:
                    raise StopIteration
            except StopIteration:
                raysYuvDepth_iter = iter(DataLoader(RayDataset(yuv_rays_depth), batch_size = N_yuv_depth, shuffle=True, num_workers=0))
                batch_yuv_depth = next(raysYuvDepth_iter).to(device)
            batch_yuv_depth = torch.transpose(batch_yuv_depth, 0, 1)
            batch_yuv_rays_depth = batch_yuv_depth[:2] # 2 x B x 3
            target_yuv_depth = batch_yuv_depth[2,:,0] # B
            # --- FIN NUEVO CÓDIGO ---
        ...
        if args.colmap_depth:
            ...
            batch_rays = torch.cat([batch_rays, batch_rays_depth], 1) # (2, 2 * N_rand, 3)  #rays_o, rays_ d de los pixeles sacados de las imagenes y de colmap juntos

        # --- INICIO NUEVO CÓDIGO ---
        N_batch_col = N_batch + N_depth
        batch_rays = torch.cat([batch_rays, batch_yuv_rays_depth], 1)
        # --- FIN NUEVO CÓDIGO ---
        ...
        if args.colmap_depth and not args.depth_with_rgb:
            ...
            disp = disp[:N_batch]
            acc = acc[:N_batch]
            # --- INICIO NUEVO CÓDIGO: división de profundidad predicha en rgb / colmap / yuv ---
            depth, depth_col, depth_yuv = depth[:N_batch], depth[N_batch:N_batch_col], depth[N_batch_col:]
            # --- FIN NUEVO CÓDIGO ---
        ...
        # --- INICIO NUEVO CÓDIGO: obtención del error de profundidad de los archivos YUV ---
        yuv_depth_loss = img2mse(depth_yuv, target_yuv_depth)
        # --- FIN NUEVO CÓDIGO ---
        ...
        # --- INICIO NUEVO CÓDIGO: se añade yuv_depth_loss a la función de pérdida ---
        loss = img_loss + args.depth_lambda * depth_loss + 0.5*yuv_depth_loss    #+ args.sigma_lambda * sigma_loss      
        ...
        if i%args.i_print==0:
            ...
            if i%1000==0:
                ...
                iterations_since_last = i - last_iteration
                iterations_per_second = iterations_since_last / elapsed_time
                monitor.log_iteration(i, psnr.item(), loss.item(), iterations_per_second)       # NUEVO
